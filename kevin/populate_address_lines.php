...
$r_haha = mysql_query("SELECT LF_NAME FROM centreline_$table_date WHERE LF_NAME != '' AND LF_NAME IS NOT NULL GROUP BY LF_NAME"); print mysql_error();
while ((list($lf_name) = mysql_fetch_row($r_haha)))
{
	$points = array();
	$r_points = mysql_query("SELECT x, y, GEO_ID FROM centreline_$table_date ".
	...
	foreach ($points as $geo_id => $geo)
	{
		...
		foreach ($geo as $i => $point)
		{
			...
			$q = "INSERT INTO address_lines (geo_id, lf_name, x, y, sequence, arc_side) values ".
				"($point->GEO_ID, \"".mysql_real_escape_string($lf_name)."\", ".$point->rx.", ".$point->ry.", $i, 'R')";
			mysql_query($q); print mysql_error();
		}
	}
}
